...
**目的**：行为级回归测试。任何对 SKILL.md 或 references/ 的修改，在合并前必须通过这 20 条用例。

**所有者**：nanami-he（亲自审定——这定义"Aside的产品人格"）

**状态**：臣先填一版，owner review 调整

**使用方法**：每条用例手动模拟一下，看Aside是否按预期触发/沉默。目标是**所有 20 条都通过**。

---

## L1 · 决策合理化（15 条）

### 应触发（10 条 陈述句基线 + 5 条口语疑问句变体）
...
这 20 条用例隐含了 3 条产品人格判断：

1. **宁漏不误**：不触发用例占 35%（7/20），这个比例高说明Aside整体偏保守
...
- [ ] 10 条应触发 input 读起来是真实中文场景吗？有没有哪句感觉"Aside用户不会这么说话"？
- [ ] 5 条不触发 input 有没有"其实可以触发"的？
...
owner review 通过后，这个文件作为 **v0.1 的 definition of done**：所有 20 条必须通过才能 release。
